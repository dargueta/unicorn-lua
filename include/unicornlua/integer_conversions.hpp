...
// We know lua_Integer is always at least 32 bits, so we don't need any checks
// here.
...
/*
 * The following are default configurations and definitions for Lua integers.
 * Note that for Lua 5.1 and 5.2, `lua_Integer` can be redefined to any type.
 * In 5.3+, it must be one of `long long`, `long`, or `int`; ptrdiff_t is no
 * longer an option.
 *
 * Lua 5.1:
 ...
 * Lua 5.2:
 *     - lua_Integer: ptrdiff_t
 *     - lua_Unsigned: `int` or `long`, whichever is smallest and >=32 bits
 *     - Defines LUA_MAXINTEGER: No
 *     - Defines LUA_MAXUNSIGNED: No
 * Lua 5.3+:
 *     - lua_Integer:
 *           - 32-bit builds: `int` or `long`, whichever is smallest and >=32
 * bits
...
// A regular Lua integer can safely hold a size_t. This will only ever be the
// case on systems where a size_t cannot hold a pointer, e.g. a 64-bit system
// with a 32-bit size_t. This is uncommon but does exist.
...
// Lua 5.1 and 5.2 use ptrdiff_t as their default integer type. On a 64-bit
// system this *might* be able to hold a 32-bit unsigned integer. Technically,
// ptrdiff_t only needs to be able to hold pointer arithmetic between pointers
// in the same array. If on a 64-bit system malloc() will only let you allocate
// 4GiB, ptrdiff_t might be a 32-bit type.
//
// Of course, this assumes that whoever built Lua didn't change it to a type
