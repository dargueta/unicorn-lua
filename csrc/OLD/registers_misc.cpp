...
constexpr uint8_t kFP80PositiveInfinity[] = {0, 0, 0, 0, 0, 0, 0, 0x80, 0xff, 0x7f};
constexpr uint8_t kFP80NegativeInfinity[] = {0, 0, 0, 0, 0, 0, 0, 0x80, 0xff, 0xff};
...
RegisterDataType Register::get_kind() const noexcept
{
    return kind_;
}

size_t Register::get_size() const
{
    return size_for_register_kind(kind_);
}
...
int ul_reg_read_as(lua_State *L)
{
    ...
    if (register_id == UC_X86_REG_MSR)
    {
        throw LuaBindingError("reg_read_as() doesn't support reading x86 model-specific"
                              " registers.");
    }
...
int ul_reg_write_batch(lua_State *L)
{
    ...
    uc_err error = uc_reg_write_batch(engine, register_ids.get(), p_values.get(),
