...
describe('Memory regions', function ()
  ...
  it('Handles one region', function ()
    local uc = unicorn.open(unicorn.UC_ARCH_X86, unicorn.UC_MODE_64)
    uc:mem_map(0, 0x100000, unicorn.UC_PROT_ALL)
    ...
    assert.are.equals(region.perms, unicorn.UC_PROT_ALL)
  end)

  it('Handles multiple regions', function ()
    local uc = unicorn.open(unicorn.UC_ARCH_X86, unicorn.UC_MODE_64)
    uc:mem_map(0, 0x100000, unicorn.UC_PROT_ALL)
    uc:mem_map(0x200000, 0x1000, unicorn.UC_PROT_EXEC)
    ...
    assert.are.equals(regions[1].perms, unicorn.UC_PROT_ALL)

    assert.are.equals(regions[2].begins, 0x200000)
    assert.are.equals(regions[2].ends, 0x200fff)
    assert.are.equals(regions[2].perms, unicorn.UC_PROT_EXEC)
  end)
end)
