...
.PHONY: autoformat
autoformat:
	clang-format --Werror -i --verbose $(filter-out $(AUTOGENERATED_CPP_FILES),$(wildcard src/*.cpp))
	clang-format --Werror -i --verbose $(filter-out $(AUTOGENERATED_HPP_FILES),$(wildcard include/unicornlua/*.hpp))
	clang-format --Werror -i --verbose $(wildcard tests/c/*.cpp)
	clang-format --Werror -i --verbose $(wildcard tests/c/*.hpp)
...
$(TEST_EXECUTABLE): $(DOCTEST_HEADER) $(TEST_CPP_OBJECT_FILES) $(LIB_OBJECT_FILES) $(TEST_HEADERS)
	$(LINK_CMD) -o $@ $(filter-out %.h %.hpp,$^) $(REQUIRED_LIBS_FLAGS) $(LINK_TO_LUA_FLAG) -lm
