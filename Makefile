...
LIB_C_SOURCES = \
    $(addprefix $(SOURCE_DIR)/,context.c control_functions.c engine.c errors.c hooks.c registers_api.c unicorn.c) \
    $(AUTOGENERATED_C_FILES)
LIB_OBJECT_FILES = $(LIB_C_SOURCES:.c=.$(OBJ_EXTENSION))

LIB_CONFIG_TEMPLATE_DATA = $(SOURCE_DIR)/template_data/libconfig.lua
TEMPLATE_DATA_FILES = $(wildcard $(SOURCE_DIR)/template_data/*.lua) $(LIB_CONFIG_TEMPLATE_DATA)
...
$(LIB_CONFIG_TEMPLATE_DATA):
	echo '-- Searched for headers inside $(UNICORN_INCDIR)/unicorn' > $@
	echo 'architecture_support = {' >> $@
	$(foreach a,$(ARCHITECTURES),\
		[ ! -f '$(UNICORN_INCDIR)/unicorn/$(a).h' ] || echo "    $(a) = true," >> $@; \
	)
	$(foreach a,$(ARCHITECTURES),\
		[ -f '$(UNICORN_INCDIR)/unicorn/$(a).h' ] || echo "    $(a) = false," >> $@; \
	)
	echo '}' >> $@


$(LUA_SOURCE_DIR)/%_extracted_consts.lua: tools/process_header.lua
...
.PHONY: clean
clean:
	if which git >/dev/null; then git clean -fdX src csrc; fi
	$(RM) $(LIB_OBJECT_FILES) $(LIB_BUILD_TARGET) $(ALL_AUTOGENERATED_SOURCES) $(LDOC_FILES) core*
...
.PHONY: docs
docs: $(ALL_AUTOGENERATED_SOURCES) $(LDOC_FILES)
	ldoc .
...
.PHONY: test
test:
	export USER_CFLAGS='$(DEBUG_CFLAGS)' && export USER_LDFLAGS='$(DEBUG_LDFLAGS)' && luarocks build
	export USER_CFLAGS='$(DEBUG_CFLAGS)' && export USER_LDFLAGS='$(DEBUG_LDFLAGS)' && luarocks test

.PHONY: debug
debug:
	if [ ! -f core* ]; then echo 'No core dumps to debug'; exit 1; fi
	gdb $(shell which lua) $(firstword $(wildcard core*))
