...
Hook::Hook(lua_State *L, uc_engine *engine, uc_hook hook_handle, int callback_func_ref,
           int user_data_ref)
...
uc_engine *Hook::engine() noexcept
{
    return engine_;
}

lua_State *Hook::L() noexcept
{
    return L_;
}

uc_hook Hook::get_hook_handle() const noexcept
{
    return hook_handle_;
}
...
int Hook::get_callback() const noexcept
{
    return callback_func_ref_;
}
...
int Hook::get_user_data() const noexcept
{
    return user_data_ref_;
}
...
static void get_callback(Hook *hook)
{
    ...
    if (lua_isnil(L, -1))
    {
        luaL_error(L, "No callback function found for hook %p attached to engine %p",
                   hook, hook->engine());
    }
}
...
static bool invalid_mem_access_hook(uc_engine *uc, uc_mem_type type, uint64_t address,
                                    int size, int64_t value, void *user_data)
{
    auto hook = reinterpret_cast<Hook *>(user_data);
    lua_State *L = hook->L();
...
static void edge_generated_hook(uc_engine *uc, uc_tb *cur_tb, uc_tb *prev_tb,
                                void *user_data)
{
    auto hook = reinterpret_cast<Hook *>(user_data);
    lua_State *L = hook->L();
...
static void tcg_opcode_hook(uc_engine *uc, uint64_t address, uint64_t arg1, uint64_t arg2,
...
int ul_hook_add(lua_State *L)
{
    ...
    if (n_args < 6)
        error = uc_hook_add(engine_handle, &hook_handle, hook_type, c_callback, hook_info,
                            start, end);
    else
        error = uc_hook_add(engine_handle, &hook_handle, hook_type, c_callback, hook_info,
                            start, end, extra_argument);
